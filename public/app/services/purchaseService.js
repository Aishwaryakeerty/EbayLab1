  ...
  angular.module('shopApp')
    .factory('PurchaseFactory', ['Product', '$http', '$window', function (Product, $http, $window) {
      return {
        product: {},
        addToCart: function () {
          this.product.image = Product.item.images;
          this.product.title = Product.item.title;
          this.product.brand = Product.item.brand;
          this.product.price = Product.item.pricing;
          this.product.details = Product.item.details;
        },
        ...
        getCartSum: function (key) {
          ...
          for (var i = 0, j = this.cartProducts.length; i < j; i++) {
            sum += parseFloat(this.cartProducts[i][key]);
          }
          this.cart.sum = sum;
          return sum;
        },
